...
export async function POST(request) {
  ...
  try {
    ...
    if (!bookingRef || !phone || !amount) {
      return NextResponse.json({ error: 'bookingRef, phone and amount are required' }, { status: 400 })
    }
    ...
    if (!tokenRes.ok) {
      const errText = await tokenRes.text().catch(() => tokenRes.status)
      throw new Error(`M-Pesa OAuth failed: ${errText}`)
    }
    ...
    const normalizedPhone = cleanPhone.replace(/\D/g, '').replace(/^0/, '254').replace(/^\+/, '')
    ...
    // 4. Store CheckoutRequestID in DB
    const booking = await prisma.booking.findFirst({ where: { bookingRef } })
    if (booking) {
      await prisma.booking.update({
        where: { id: booking.id },
        data:  { mpesaCheckoutId: stkData.CheckoutRequestID, paymentStatus: 'PENDING' },
      })
    }
